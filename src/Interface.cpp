...
static const GLfloat crossVertices[] =
{
...
void Interface::draw()
{
    drawCross();
    drawXZ();
    drawText("Open source URL: https://github.com/BuleStorm", Width / 4, 40, 1.0f, glm::vec3(0.11f, 0.56f, 1.00f));
}
...
void Interface::initLoginBackground()
{
    ...
    textureB = loadTexture("./resource/Login/bg1.jpg");
}
...
void Interface::drawLoginBackground()
{
    shaderB.use();
    glm::mat4 projection = glm::ortho(0.0f, static_cast<GLfloat>(Width), 0.0f, static_cast<GLfloat>(Height));
    glUniformMatrix4fv(glGetUniformLocation(shaderB.ID, "projection"), 1, GL_FALSE, glm::value_ptr(projection));

    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, textureB);
    glUniform1i(glGetUniformLocation(shaderB.ID, "texture1"), 0);
...
void Interface::drawXZ()
{
    string posStr = "Position:";
    posStr.append(to_string(int(control->eyePos.x)));
    posStr.append(",");
    posStr.append(to_string(int(control->eyePos.z)));
    drawText(posStr.c_str(), 0.04 * Width, 0.86 * Height, 1.0f , glm::vec3(1.00f, 1.00f, 1.00f));
